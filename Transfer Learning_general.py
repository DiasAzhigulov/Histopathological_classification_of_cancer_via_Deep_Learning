...
BASE_PATH = ['datasets/TMAD/','datasets/breakhis_m_subtypes_aug/','datasets/breakhis_b_subtypes_aug/','datasets/breakhis_binary/']
...
for bp in BASE_PATH:
	...
	model_names1 = ['DenseNet121']

	for name in model_names1:
		print(i)
		if(i==0 and name=='InceptionV3'):
			continue
		...
		mcp_save = keras.callbacks.callbacks.ModelCheckpoint(str(i)+"_new_Best_model#"+str(NUM_EPOCHS)+'_'+ str(name) + ".h5",monitor='val_acc', verbose=0, 
		...
		model.save(str(i)+"_new_AllLayer_model#"+str(NUM_EPOCHS)+'_'+ str(name) + ".h5")
		print("Saved model to disk")

		

		testGen.reset()
		model = load_model(str(i)+"_new_Best_model#"+str(NUM_EPOCHS)+'_'+ str(name) + ".h5")
		...
		auc = roc_auc_score(testGen.classes,predIdxs)
		...
		fname1 = str(i)+"_new_AllLayer_modelHistory#"+str(NUM_EPOCHS)+'_'+ str(name) + ".txt"
		f = open(fname1,"w+")
		f.write("\nValidation acc: " + str(round(H.history['val_acc'][NUM_EPOCHS-1],4)) + "; Training acc: " + str(round(H.history['acc'][NUM_EPOCHS-1],4)) + 
			"\nSensitivity (avg): " + str(round(sensitivity,4)) + 
			"; Test acc: " + str(round(acc,4)) + "; AUC: " +str(round(auc,4)))
		...
		plt.savefig(str(i)+'_new_AllLayer_modelHistory#'+str(NUM_EPOCHS)+'_'+ str(name)+'_accuracy.png')
		...
		plt.savefig(str(i)+'_new_AllLayer_modelHistory#'+str(NUM_EPOCHS)+'_'+ str(name)+'_loss.png')
